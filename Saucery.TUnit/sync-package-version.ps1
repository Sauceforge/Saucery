...
# --- Parse XML only to READ TUnit version ---
...
$tunit = $pkgRefs | Where-Object { $_.Include -eq 'TUnit' } | Select-Object -First 1
if (-not $tunit) { Write-Error "TUnit PackageReference not found."; exit 1 }

# Prefer Version attribute, then <Version> child
$tunitVersion = $null
if ($tunit.Attributes['Version']) { $tunitVersion = $tunit.Attributes['Version'].Value }
if ([string]::IsNullOrWhiteSpace($tunitVersion)) {
    $verNode = $tunit.SelectSingleNode('Version')
    if ($verNode -and -not [string]::IsNullOrWhiteSpace($verNode.InnerText)) {
        $tunitVersion = $verNode.InnerText
    }
}
if ([string]::IsNullOrWhiteSpace($tunitVersion)) {
    Write-Error "Could not determine TUnit version (no Version=... attribute or <Version> child)."
    exit 1
}
...
if ($before -eq $tunitVersion) {
    Write-Host "No change needed. <PackageVersion> already $tunitVersion"
    exit 0
}

# Manual single-replace to avoid touching anything else
$prefix  = $originalText.Substring(0, $match.Index)
$middle  = $match.Groups[1].Value + $tunitVersion + $match.Groups[3].Value
...
if ($after -ne $tunitVersion) {
    Write-Error "Save verification failed. Expected '$tunitVersion', found '$after'."
    exit 1
}

Write-Host "Updated <PackageVersion> $before -> $tunitVersion in '$CsprojPath' (encoding & BOM preserved exactly)."
